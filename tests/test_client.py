...
def test_redirect_to_login_raises_session_expired(
    respx_mock: respx.MockRouter, make_settings: Callable[..., Settings]
) -> None:
    respx_mock.get("https://app.exodus90.com/today").mock(
        return_value=httpx.Response(302, headers={"location": "/auth/login?src=%2Ftoday"})
    )
    with ExodusClient(make_settings()) as client, pytest.raises(SessionExpiredError):
        client.get("/today")


def test_ok_response_is_returned(
    respx_mock: respx.MockRouter, make_settings: Callable[..., Settings]
) -> None:
    respx_mock.get("https://app.exodus90.com/today").mock(
        return_value=httpx.Response(200, text="<html>ok</html>")
    )
    with ExodusClient(make_settings()) as client:
...
def test_is_authenticated_false_on_redirect(
    respx_mock: respx.MockRouter, make_settings: Callable[..., Settings]
) -> None:
    respx_mock.get("https://app.exodus90.com/today").mock(
        return_value=httpx.Response(302, headers={"location": "/auth/login?src=%2Ftoday"})
